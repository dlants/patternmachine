...
Elm.PatternMachine.make = function (_elm) {
   ...
   var drawAddBox = F2(function (address,clickAction) {
      return A2($Html.div,
      _U.list([A2($Html$Events.onClick,address,clickAction)
              ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "background-color",_1: "rgba(0,0,0,0.25)"},{ctor: "_Tuple2",_0: "color",_1: "white"}]))]),
      _U.list([$Html.text("+")]));
   });
   ...
   var drawBox = F5(function (address,size,rowidx,colidx,val) {
      return A2($Html.div,_U.list([A2($Html$Events.onClick,address,A2(Increment,rowidx,colidx)),A2(boxStyle,val,size)]),_U.list([]));
   });
   var drawRow = F4(function (address,size,rowidx,row) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "display",_1: "flex"}]))]),
      $Array.toList(A2($Array.indexedMap,A3(drawBox,address,size,rowidx),row)));
   });
   var boxsize = function (model) {
      var _p0 = model.matrix.size;
      var cols = _p0._0;
      var rows = _p0._1;
   ...
   var addColumn = function (matrix) {
      return A2($Maybe.withDefault,matrix,A2($Matrix.concatHorizontal,matrix,A3($Matrix.repeat,1,$Matrix.height(matrix),0)));
   };
   var update = F2(function (action,model) {
      var newModel = function () {
         ...
         {case "NoOp": return model;
            case "Increment": return _U.update(model,{matrix: A4($Matrix.update,_p2._1,_p2._0,function (val) {    return val + 1;},model.matrix)});
            case "AddRow": return _U.update(model,{matrix: addRow(model.matrix)});
            case "AddColumn": return _U.update(model,{matrix: addColumn(model.matrix)});
            default: return _U.update(model,{window: _p2._0});}
      }();
      return {ctor: "_Tuple2",_0: newModel,_1: $Effects.none};
   });
   var matrixRows = function (matrix) {
      return A2($List.filterMap,function (i) {    return A2($Matrix.getRow,i,matrix);},_U.range(0,$Matrix.height(matrix) - 1));
   };
   var drawMatrix = F3(function (address,size,matrix) {    return A2($Html.div,_U.list([]),A2($List.indexedMap,A2(drawRow,address,size),matrixRows(matrix)));});
